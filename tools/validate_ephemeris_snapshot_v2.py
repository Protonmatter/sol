...
def check_bodies(bodies: list[Any], time: Any, observer: Any) -> list[str]:
    ...
    for index, body in enumerate(bodies):
        ...
        if (body.get("transit_jd") is None) != (body.get("transit_alt_deg") is None):
            errors.append(f"{prefix}.transit_jd and transit_alt_deg must both be null or both numeric")
        for event_name in ("rise_jd", "transit_jd", "set_jd"):
            ...
            if event is not None and finite(event) and day_start is not None:
                if not day_start <= float(event) < day_start + 1.0:
                    errors.append(
