...
def rings_of(geometry: dict) -> list[list[list[float]]]:
    """Every linear ring in a Polygon/MultiPolygon, outer and inner alike."""
...
def load_rings(filename: str) -> list[list[int]]:
    doc = json.loads((SRC / filename).read_text(encoding="utf-8"))
    out = []
    for feat in doc["features"]:
        for ring in rings_of(feat["geometry"]):
            enc = encode_ring(ring)
            if len(enc) >= 8:  # at least 4 distinct points
                out.append(enc)
    return out
...
# ------------------------------------------------------------------ emit
def js_int_array(rings: list[list[int]], indent: str) -> str:
    return ",\n".join(f"{indent}[{','.join(str(v) for v in r)}]" for r in rings)
...
def build() -> str:
    land = load_rings("ne_110m_land.geojson")
    lakes = load_rings("ne_110m_lakes.geojson")
    ice = load_rings("ne_110m_glaciated_areas.geojson")
    feats = load_features()

    total_pts = sum(len(r) // 2 for r in land + lakes + ice)
    parts = [
        ...
        "//\n",
        "// Earth rings are Natural Earth 1:110m vectors, quantised to 1/%d degree and delta-encoded\n" % QUANT,
        "// as [lon0, lat0, dlon, dlat, ...] in integer units — decode with decodeRing().\n",
        ...
        "}\n",
        "\n",
        f"// {len(land)} coastline rings, {len(lakes)} lake rings, {len(ice)} ice rings ({total_pts} points).\n",
        "export const EARTH = {\n",
        "  land: [\n", js_int_array(land, "    "), "\n  ],\n",
        "  lakes: [\n", js_int_array(lakes, "    "), "\n  ],\n",
        "  ice: [\n", js_int_array(ice, "    "), "\n  ],\n",
