...
try{
  ...
  evidence.gpu=await page.evaluate(async({columns,qualifyTerrain})=>{
    ...
    const begin='void main(){int x=int(gl_FragCoord.x);vec4 a=texelFetch(u_queries,ivec2(x,0),0);vec4 b=texelFetch(u_queries,ivec2(x,1),0);bool limb=a.a>0.5;vec3 delta=limb?b.xyz:a.xyz-u_atmosphereCameraKm;float maximum=limb?b.a:length(delta);vec3 ray=normalize(delta);outDomain=vec4(atmosphereRayInterval(u_atmosphereCameraKm,ray,u_atmosphereRadiusKm),atmosphereRayInterval(u_atmosphereCameraKm,ray,u_atmosphereRadiusKm+u_atmosphereTopKm));';
    const reference=program(prefix+legacy.ATMOSPHERE_RENDER_GLSL+declarations+begin+'AtmosphereResult r=integrateAtmosphere(u_atmosphereCameraKm,ray,maximum);outS=vec4(r.scattering,1);outT=vec4(r.transmittance,1);outColor=vec4(vec3(.18)*r.transmittance+r.scattering,1);}');
    const actual=program(prefix+scattering.ATMOSPHERE_SCATTERING_GLSL+declarations+begin+'vec4 s=limb?atmosphereLimbScattering(ray):atmosphereSurfaceScattering(a.xyz);vec3 t=atmosphereViewTransmission(u_atmosphereCameraKm,ray,maximum);outS=s;outT=vec4(t,1);outColor=vec4(limb?vec3(.18)*t+s.rgb:atmosphereSurfaceColor(vec3(.18),a.xyz),1);}');
    const actualExplicit=qualifyTerrain?program(prefix+scattering.ATMOSPHERE_SCATTERING_GLSL+declarations+begin+'vec4 h=texelFetch(u_queries,ivec2(x,2),0);vec4 s=limb?atmosphereLimbScattering(ray):atmosphereSurfaceScattering(a.xyz,h.x);vec3 t=atmosphereViewTransmission(u_atmosphereCameraKm,ray,maximum);outS=s;outT=vec4(t,1);outColor=vec4(limb?vec3(.18)*t+s.rgb:atmosphereSurfaceColor(vec3(.18),a.xyz,h.x),1);}'):null;
