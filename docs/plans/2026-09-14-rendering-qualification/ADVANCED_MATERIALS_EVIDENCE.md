...
| D3 finer terrain | New Moon/Mars numerical height products drive displacement, normals and the full shadow grid; close views select level 4. | Exhaustive source/derivative comparison, allocation/cancellation checks and actual level 4 draw submissions; [final native/software application captures](RUNTIME_FINAL_QUALIFICATION.md) pass. | Further devices and sustained performance remain outside the finite current-host captures. Regional tiled terrain and finer photographic color products have not been implemented. |
