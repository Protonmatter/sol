...
| Earth historical cloud layer and dated weather swaths: mode-3 auxiliary | Same registered grid; the historical cloud layer carries white RGB with alpha mapped from its grayscale source by a pinned curve fitted to NASA's 2002 composite, daily swaths have alpha no-data; both upload premultiplied with ordinary encoded filtering | `mix(surfaceLinear,D(coveredRGB(weather)),weather.a)` **before** illumination | A disabled/unready selected source has its flag cleared. Its date is not replaced by another source. The historical layer is clouds only over the land map; daily swaths are clouds-and-surface display imagery. Neither is an inferred cloud density layer. |
