...
async function fixture({datum=0}={}){
  ...
  let unit=0,settled=0;const producer={configuration:null,passed:true,configure(value){this.configuration=value;},capture(){return {passed:this.passed,reason:'missing producer',passes:[0,1]};}};
  const calls=[];
  class GL{
  ...
  const context=vm.createContext({WebGL2RenderingContext:GL,crypto:webcrypto,TextEncoder,URL,
    ...
    __solScatteringProducerEvidence:producer,
  ...
  const uniforms={u_scatteringReferenceHeightKm:Math.fround(datum),u_atmosphereEnabled:1,u_atmosphereRefractionEnabled:1,u_incidentFieldReady:1,u_atmosphereRadiusKm:1,u_bodyRadiusKm:Math.fround(1+datum),u_atmosphereTopKm:2,
    ...
    u_incidentField:6,u_atmosphereColumnField:7,u_atmosphereCameraKm:[0,0,Math.fround(3*(1+datum))],u_atmosphereSunDirection:[-1,0,0],u_incidentFieldHeight:[0,16,1],
  ...
  link();
  return {gl,program,uniforms,uploads,textures,summary,store,calls,link,context,producer,capture:()=>context.__solPhysicalSpinEvidence.capture(gl,program),settled:()=>settled,unit:()=>unit};
}
...
test('physical draw requires observed production sources after shader companions detach',async()=>{
  ...
  assert.equal(f.producer.configuration.vertexSource,'generator vertex');assert.equal(result.scattering.passes.length,2);
...
test('physical draw rejects fallback, pending refraction and wrong current source/profile bindings',async()=>{
  for(const change of [f=>f.uniforms.u_atmosphereEnabled=0,f=>f.uniforms.u_atmosphereRefractionEnabled=0,
    f=>f.uniforms.u_scatteringReferenceHeightKm=.01,f=>f.producer.passed=false,f=>f.uniforms.u_incidentFieldReady=0,f=>f.uniforms.u_bodyRadiusKm=2,f=>f.uniforms.u_atmosphereExposure=2,
...
test('physical surface datum is an independently observed scalar from the catalogue/profile difference',async()=>{
  const f=await fixture({datum:.003});const result=f.capture();assert.equal(result.passed,true,result.reason);
  assert.equal(result.uniforms.u_scatteringReferenceHeightKm,Math.fround(.003));
  assert.ok(Math.abs(result.scattering_height_range_km[0]-.003)<1e-15);
  f.uniforms.u_scatteringReferenceHeightKm=0;assert.equal(f.capture().passed,false);
});
