//! Frozen v2 snapshot-copy validation. This does not restore transport checkpoints.
...
pub fn validate(raw: &str) -> Result<(), String> {
    ...
    if value.get("schema_version").and_then(JsonValue::as_str) != Some("solar-state-snapshot.v2") {
        return Err("not a solar-state-snapshot.v2 file".into());
    }
    if value
        .get("coordinates")
    ...
    .map_err(|err| format!("embedded v2 schema: {err}"))?;
...
fn semantics(value: &JsonValue) -> Result<(), String> {
    ...
    for key in [
        ...
        for v in values {
            ...
            if key == "br_variance_normalized" && n < 0.0
                || key == "confidence" && !(0.0..=1.0).contains(&n)
            {
    ...
    for region in regions {
        if number(region, "birth_seconds")? > time + 1e-6 {
            return Err("active_regions birth_seconds is in the future".into());
        }
    }
...
mod tests {
    ...
    fn shared_snapshot_mutation_corpus() {
        let baseline = parse_json(include_str!(
            "../../../tests/fixtures/historical/solar-v2.json"
        ))
        .unwrap();
        let cases = parse_json(include_str!(
            "../../../tests/fixtures/historical/snapshot-intake-v2.json"
        ))
        .unwrap();
