...
export function layoutLabels(candidates, {width, height, clearance = 4, limit = width < 600 ? 8 : 16}) {
  if (![width,height,clearance,limit].every(Number.isFinite) || width <= 2*clearance || height <= 2*clearance || clearance < 0 || limit < 0) return [];
  ...
  const fits = box => box.x >= clearance && box.y >= clearance && box.x+box.width <= width-clearance
  ...
  for (const c of ordered) {
    ...
    if (!box && c.priority <= 2) {
      ...
      if (!fits(box)) {
        box = null;
        for (let row=clearance; row+h<=height-clearance; row+=h+clearance) {
