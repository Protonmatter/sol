// @lazy-module Historical v2 copies only; never imported by live providers.
...
export function assertEphemerisSnapshotV2(value) {
  ...
  const names = new Set();
  const offsetDays = observer.terrestrial_lon_deg_east / 360;
  const dayStart = Math.floor(time.jd_utc - 0.5 + offsetDays) + 0.5 - offsetDays;
  snapshot.bodies.forEach((entry, index) => {
    const body = bodyAt(entry, index);
    if ((body.transit_jd === null) !== (body.transit_alt_deg === null)) {
      fail(`$.bodies[${index}].transit_jd`, "transit_jd and transit_alt_deg must both be null or both numeric");
    }
    for (const field of ["rise_jd", "transit_jd", "set_jd"]) {
      if (body[field] !== null && !(body[field] >= dayStart && body[field] < dayStart + 1)) {
        fail(`$.bodies[${index}].${field}`, "outside the observer local mean-solar day");
      }
    }
