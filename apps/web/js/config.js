...
export const GLOSSARY = {
  ...
  "tidal-locking": ["Tidal locking", "When a body's rotation period has been dragged into step with its orbital period, so it keeps the same face toward its partner. The Moon is locked to Earth. Note what this does NOT mean: a locked body still turns once per orbit as seen from the stars, which is exactly why the Moon visibly rotates in this view — the locking is only relative to Earth."],
  "libration": ["Libration", "The Moon's slight monthly rocking as seen from Earth. It travels its elliptical orbit at a varying speed while spinning at a constant one, so it runs alternately ahead and behind (libration in longitude), and its axis is tilted to its orbit, so we peek over each pole in turn (libration in latitude). Together they let us see about 59% of the lunar surface instead of exactly half."],
  "parallax-distance": ["Parallax distance", "A star's distance measured by geometry, not inference: as Earth orbits, a nearby star appears to shift slightly against far-off ones, and that tiny angle gives the distance directly. Hipparcos measured it for these stars. The further the star, the smaller the shift and the larger the uncertainty."],
  "colour-index": ["Colour index (B−V)", "A star's brightness in blue light minus its brightness in visible light. Because hot stars glow blue and cool stars glow red, this single number is a thermometer: negative is hot and blue, around 0.65 is Sun-like yellow, and above 1.4 is cool and red."],
  "spectral-type": ["Spectral type", "The classification of a star from the lines in its spectrum, ordered hottest to coolest as O, B, A, F, G, K, M. The Sun is a G star. The trailing digit subdivides each class, and a roman numeral (when present) gives the luminosity class."],
  "absolute-magnitude": ["Absolute magnitude", "How bright a star would look from a standard distance of 32.6 light-years (10 parsecs). Unlike apparent magnitude it strips away distance, so it compares stars by their true output: Sirius looks brightest in our sky, but Deneb is vastly more luminous and merely far away."],
