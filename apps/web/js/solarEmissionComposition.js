...
void main(){
  ivec2 xy=ivec2(gl_FragCoord.xy-u_origin);vec3 fields=texelFetch(u_transfer,xy,0).rgb;
...
export function createSolarEmissionComposition(gl,{generation=1,maxBytes=48*1024*1024}={}) {
  ...
  async function ready(){
    ...
    const program=programs.get('compose');uniforms=Object.fromEntries(['u_transfer','u_depth','u_pass','u_linearOutput','u_channel','u_origin','u_exposure'].map(name=>[name,gl.getUniformLocation(program,name)]));
  }
  ...
  function begin(){
    ...
    try{
      gl.activeTexture(gl.TEXTURE0);resize(saved.viewport[2],saved.viewport[3]);
