...
export function systemCard(state = {}) {
  ...
  const appearance=name==='Sun'&&state.solarMode
    ? state.solarMode.startsWith('dynamic-')?`Illustrative full-sphere ${state.solarMode==='dynamic-visible'?'photosphere':'corona in assigned 171 Å-style gold'}. Statistical surface evolution and a synthetic potential magnetic field. Relative emission, not observed far-side imagery, calibrated plasma, or a space-weather prediction. ${state.solarDynamicStatus?.state==='ready'?'Model time is independent of planetary time.':state.solarDynamicStatus?.reason||'Select The Sun to load the model.'}`
